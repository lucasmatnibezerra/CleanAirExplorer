...
export function DashboardPage(){
  ...
  const trendLabel = delta === 0 ? t('common.stable','Stable') : delta > 0 ? `${t('common.rising','Rising')} +${delta}` : `${t('common.falling','Falling')} ${delta}`
  return (
  ...
  <KpiCard label={t('dashboard.currentAqi','Current AQI')} value={current? current.aqi: '—'} sub={current? t(aqiCategory(current.aqi, {key:true}) as string): t('aqi.noData','No data')} intent={current? (current.aqi<=50? 'good': current.aqi<=100? 'moderate': current.aqi<=150? 'warning': 'unhealthy'):'neutral'} />
  <KpiCard label={t('dashboard.dominant','Dominant')} value={dominant} sub={t('dashboard.pollutant','Pollutant')} />
  <KpiCard label={t('dashboard.trend24h','24h Trend')} value={trendLabel} sub={t('dashboard.vsPrevHour','vs prev hour')} intent={delta>0? 'warning': delta<0? 'good':'neutral'} />
  <KpiCard label={t('dashboard.wind','Wind')} value={"3.4 m/s"} sub={t('dashboard.nowcast','Nowcast')} />
      </div>
